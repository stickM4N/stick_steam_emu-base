...
public:
	...
	/// call it directly
	STEAM_PRIVATE_API(virtual bool InitGameServer(
	                      uint32 unIP, uint16 usGamePort, uint16 usQueryPort,
	...
	/// Spectator server port to advertise.  The default value is zero, meaning
	/// the service is not used.  If your server receives any info requests on
	/// the LAN, this is the value that will be placed into the reply for such
	/// local queries.
	///
	/// This is also the value that will be advertised by the master server.
	/// The only exception is if your server is using a FakeIP.  Then then the
	/// second fake port number (index 1) assigned to your server will be listed
	/// on the master server as the spectator port, if you set this value to any
	/// nonzero value.
	///
	/// This function merely controls the values that are advertised -- it's up
	/// to you to configure the server to actually listen on this port and
	/// handle any spectator traffic
	...
	///
	/// (This function used to be named EnableHeartbeats, so if you are
	/// wondering where that function went, it's right here.  It does the same
	/// thing as before, the old name was just confusing.)
	...
	// Server browser related query packet processing for shared socket mode.
	// These are used when you pass STEAMGAMESERVER_QUERY_PORT_SHARED as the
	// query port to SteamGameServer_Init. IP address and port are in host
	// order, i.e 127.0.0.1 == 0x7f000001
	...
	ComputeNewPlayerCompatibility(CSteamID steamIDNewPlayer)
	    = 0;
	...
	// should call EndAuthSession() when this user leaves the server just like
	// you would for a real user.
	...
	// Deprecated functions.  These will be removed in a future version of the
	// SDK. If you really need these, please contact us and help us understand
	// what you are using them for.

	STEAM_PRIVATE_API(virtual void SetMasterServerHeartbeatInterval_DEPRECATED(
...
struct GSReputation_t {
	...
	uint32 m_unBanExpires;   // Time the ban expires, expressed in the Unix
	                         // epoch (seconds since 1/1/1970)
};
