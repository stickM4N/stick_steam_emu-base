...
enum ESteamAPICallFailure {
	...
	k_ESteamAPICallFailureNetworkFailure
	    = 1,   // the network connection to Steam has been broken, or was
	           // already broken
...
enum EFloatingGamepadTextInputMode {
	k_EFloatingGamepadTextInputModeModeSingleLine
	    = 0,   // Enter dismisses the keyboard
...
public:
	...
	// Steam server time.  Number of seconds since January 1, 1970, GMT (i.e
	// unix time)
	...
	STEAM_PRIVATE_API(virtual bool GetCSERIPPort(uint32 *unIP, uint16 *usPort)
	                  = 0;)
	...
	virtual void
	SetOverlayNotificationPosition(ENotificationPosition eNotificationPosition)
	    = 0;
	...
	virtual bool IsAPICallCompleted(SteamAPICall_t hSteamAPICall,
	                                bool *pbFailed)
	    = 0;
	virtual ESteamAPICallFailure
	GetAPICallFailureReason(SteamAPICall_t hSteamAPICall)
	    = 0;
	...
	// Deprecated. Applications should use SteamAPI_RunCallbacks() instead. Game
	// servers do not need to call this function.
	...
	// Activates the full-screen text input dialog which takes a initial text
	// string and returns the text the user has typed
	...
	// Games much be launched through the Steam client to enable the Big Picture
	// overlay. During development, a game can be added as a non-steam game to
	// the developers library to test this feature
	...
	// If this is set to true, then the scene in the HMD headset will be
	// streamed, and remote input will not be allowed. If this is set to false,
	// then the application window will be streamed instead, and remote input
	// will be allowed. The default is true unless "VRHeadsetStreaming" "0" is
	// in the extended appinfo for a game. (this is useful for games that have
	// asymmetric multiplayer gameplay)
	virtual void SetVRHeadsetStreamingEnabled(bool bEnabled) = 0;

	// Returns whether this steam client is a Steam China specific client, vs
	// the global client.
	virtual bool IsSteamChinaLauncher() = 0;

	// Initializes text filtering, loading dictionaries for the language the
	// game is running in.
	//   unFilterOptions are reserved for future use and should be set to 0
	// Returns false if filtering is unavailable for the game's language, in
	// which case FilterText() will act as a passthrough.
	//
	// Users can customize the text filter behavior in their Steam Account
	// preferences:
	...
	// pchOutFilteredText, using legally required filtering and additional
	// filtering based on the context and user settings
	//   eContext is the type of content in the input string
	//   sourceSteamID is the Steam ID that is the source of the input string
	//   (e.g. the player with the name, or who said the chat text) pchInputText
	//   is the input string that should be filtered, which can be ASCII or
	//   UTF-8 pchOutFilteredText is where the output will be placed, even if no
	//   filtering is performed nByteSizeOutFilteredText is the size (in bytes)
	//   of pchOutFilteredText, should be at least strlen(pchInputText)+1
	// Returns the number of characters (not bytes) filtered
	virtual int FilterText(ETextFilteringContext eContext,
	                       CSteamID sourceSteamID, const char *pchInputMessage,
	                       char *pchOutFilteredText,
	                       uint32 nByteSizeOutFilteredText)
	    = 0;

	// Return what we believe your current ipv6 connectivity to "the internet"
	// is on the specified protocol. This does NOT tell you if the Steam client
	// is currently connected to Steam via ipv6.
	virtual ESteamIPv6ConnectivityState
	GetIPv6ConnectivityState(ESteamIPv6ConnectivityProtocol eProtocol)
	    = 0;

	// returns true if currently running on the Steam Deck device
	virtual bool IsSteamRunningOnSteamDeck() = 0;

	// Opens a floating keyboard over the game content and sends OS keyboard
	// keys directly to the game. The text field position is specified in pixels
	// relative the origin of the game window and is used to position the
	// floating keyboard in a way that doesn't cover the text field
	...
	// In game launchers that don't have controller support you can call this to
	// have Steam Input translate the controller input into mouse/kb to navigate
	// the launcher
...
STEAM_DEFINE_INTERFACE_ACCESSOR(
    ...
    SteamInternal_FindOrCreateUserInterface(0, STEAMUTILS_INTERFACE_VERSION),
    "user", STEAMUTILS_INTERFACE_VERSION);

// Global accessor for the gameserver client
inline ISteamUtils *SteamGameServerUtils();
STEAM_DEFINE_INTERFACE_ACCESSOR(ISteamUtils *, SteamGameServerUtils,
                                SteamInternal_FindOrCreateGameServerInterface(
                                    0, STEAMUTILS_INTERFACE_VERSION),
