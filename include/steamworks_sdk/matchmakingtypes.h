...
//
// Max size (in bytes of UTF-8 data, not in characters) of server fields,
// including null terminator. WARNING: These cannot be changed easily, without
// breaking clients using old interfaces.
//
...
struct MatchMakingKeyValuePair_t {
	...
	MatchMakingKeyValuePair_t(const char *pchKey, const char *pchValue) {
		strncpy(m_szKey, pchKey,
		        sizeof(m_szKey));   // this is a public header, use basic c
		                            // library string funcs only!
...
enum EMatchMakingServerResponse {
	...
	eNoServersListedOnMasterServer   // for the Internet query type, returned in
	                                 // response callback if no servers of this
	                                 // type match
};

// servernetadr_t is all the addressing info the serverbrowser needs to know
// about a game server, namely: its IP, its connection port, and its query port.
...
public:
	...
	// This gets the 'a.b.c.d:port' string with the connection port (instead of
	// the query port).
...
inline void servernetadr_t::Init(unsigned int ip, uint16 usQueryPort,
...
inline const char *servernetadr_t::ToString(uint32 unIP, uint16 usPort) const {
	...
	_snprintf(s[nBuf], sizeof(s[nBuf]), "%u.%u.%u.%u:%i", (int)(ipByte[0]),
	          (int)(ipByte[1]), (int)(ipByte[2]), (int)(ipByte[3]), usPort);
#else
	_snprintf(s[nBuf], sizeof(s[nBuf]), "%u.%u.%u.%u:%i", (int)(ipByte[3]),
	          (int)(ipByte[2]), (int)(ipByte[1]), (int)(ipByte[0]), usPort);
#endif
...
inline bool servernetadr_t::operator<(const servernetadr_t &netadr) const {
	return (m_unIP < netadr.m_unIP)
	       || (m_unIP == netadr.m_unIP && m_usQueryPort < netadr.m_usQueryPort);
}
...
public:
	servernetadr_t
	    m_NetAdr;   ///< IP/Query Port/Connection Port for this server
	int m_nPing;   ///< current ping time in milliseconds
	bool m_bHadSuccessfulResponse;   ///< server has responded successfully in
	                                 ///< the past
	bool m_bDoNotRefresh;   ///< server is marked as not responding and should
	...
	char m_szGameDescription
	    [k_cbMaxGameServerGameDescription];   ///< game description
	uint32 m_nAppID;   ///< Steam App ID of this server
	int m_nPlayers;   ///< total number of players currently on the server.
	                  ///< INCLUDES BOTS!!
	int m_nMaxPlayers;   ///< Maximum players that can join this server
	int m_nBotPlayers;   ///< Number of bots (i.e simulated players) on this
	                     ///< server
	bool m_bPassword;   ///< true if this server needs a password to join
	bool m_bSecure;   ///< Is this server protected by VAC
	uint32
	    m_ulTimeLastPlayed;   ///< time (in unix time) when this server was last
	                          ///< played on (for favorite/history servers)
...
public:
	...
	/// steamID of the game server - invalid if it's doesn't have one (old
	/// server, or not connected to Steam)
	CSteamID m_steamID;
};


inline gameserveritem_t::gameserveritem_t() {
	m_szGameDir[0] = m_szMap[0] = m_szGameDescription[0] = m_szServerName[0]
	    = 0;
	m_bHadSuccessfulResponse = m_bDoNotRefresh = m_bPassword = m_bSecure
	    = false;
	m_nPing = m_nAppID = m_nPlayers = m_nMaxPlayers = m_nBotPlayers
	    = m_ulTimeLastPlayed = m_nServerVersion = 0;
	m_szGameTags[0] = 0;
}
