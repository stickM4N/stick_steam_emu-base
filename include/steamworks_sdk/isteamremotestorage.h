...
//-----------------------------------------------------------------------------
// Purpose: Structure that contains an array of const char * strings and the
...
const PublishedFileUpdateHandle_t k_PublishedFileUpdateHandleInvalid
    = 0xffffffffffffffffull;

// Handle for writing to Steam Cloud
typedef uint64 UGCFileWriteStreamHandle_t;
const UGCFileWriteStreamHandle_t k_UGCFileStreamHandleInvalid
    = 0xffffffffffffffffull;
...
enum EWorkshopFileType {
	...
	k_EWorkshopFileTypeMerch = 11,   // Workshop merchandise meant to be voted
	                                 // on for the purpose of being sold
	...
	k_EWorkshopFileTypeGameManagedItem
	    = 15,   // managed completely by the game, not the user, and not shown
	            // on the web
...
enum EUGCReadAction {
	// Keeps the file handle open unless the last byte is read.  You can use
	// this when reading large files (over 100MB) in sequential chunks. If the
	// last byte is read, this will behave the same as k_EUGCRead_Close.
	// Otherwise, it behaves the same as k_EUGCRead_ContinueReading. This value
	// maintains the same behavior as before the EUGCReadAction parameter was
	// introduced.
	k_EUGCRead_ContinueReadingUntilFinished = 0,

	// Keeps the file handle open.  Use this when using UGCRead to seek to
	// different parts of the file. When you are done seeking around the file,
	// make a final call with k_EUGCRead_Close to close it.
	k_EUGCRead_ContinueReading = 1,

	// Frees the file handle.  Use this when you're done reading the content.
	// To read the file from Steam again you will need to call UGCDownload
	// again.
	k_EUGCRead_Close = 2,
};
...
enum ERemoteStorageFilePathType {
	...
	// The file is accessed via the ISteamRemoteStorage API and this is the
	// filename
...
public:
	...
	// file operations
	virtual bool FileWrite(const char *pchFile, const void *pvData,
	                       int32 cubData)
	    = 0;
	...
	virtual UGCFileWriteStreamHandle_t FileWriteStreamOpen(const char *pchFile)
	    = 0;
	virtual bool
	FileWriteStreamWriteChunk(UGCFileWriteStreamHandle_t writeHandle,
	...
	CreatePublishedFileUpdateRequest(PublishedFileId_t unPublishedFileId)
	    = 0;
	virtual bool
	UpdatePublishedFileFile(PublishedFileUpdateHandle_t updateHandle,
	                        const char *pchFile)
	    = 0;
	virtual bool
	UpdatePublishedFilePreviewFile(PublishedFileUpdateHandle_t updateHandle,
	    ...
	    = 0;
	virtual bool
	UpdatePublishedFileTitle(PublishedFileUpdateHandle_t updateHandle,
	                         const char *pchTitle)
	    = 0;
	virtual bool
	...
	virtual bool UpdatePublishedFileVisibility(
	    ...
	    = 0;
	virtual bool
	UpdatePublishedFileTags(PublishedFileUpdateHandle_t updateHandle,
	...
	virtual SteamAPICall_t
	DeletePublishedFile(PublishedFileId_t unPublishedFileId)
	    = 0;
	...
	virtual SteamAPICall_t
	SubscribePublishedFile(PublishedFileId_t unPublishedFileId)
	    = 0;
	...
	virtual SteamAPICall_t
	UnsubscribePublishedFile(PublishedFileId_t unPublishedFileId)
	    = 0;
	...
	virtual const char *
	GetLocalFileChange(int iFile, ERemoteStorageLocalFileChange *pEChangeType,
...
struct RemoteStorageFileShareResult_t {
	...
	UGCHandle_t
	    m_hFile;   // The handle that can be shared with users and features
	char m_rgchFilename[k_cchFilenameMax];   // The name of the file that was
	                                         // shared
};
...
struct RemoteStorageEnumerateUserPublishedFilesResult_t {
	...
	PublishedFileId_t
	    m_rgPublishedFileId[k_unEnumeratePublishedFilesMaxResults];
};
...
struct RemoteStorageDownloadUGCResult_t {
	...
	EResult m_eResult;   // The result of the operation.
	UGCHandle_t m_hFile;   // The handle to the file that was attempted to be
	                       // downloaded.
...
//-----------------------------------------------------------------------------
// Purpose: User subscribed to a file for the app (from within the app or on the
// web)
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: User unsubscribed from a file for the app (from within the app or on
// the web)
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Published file that a user owns was deleted (from within the app or
// the web)
//-----------------------------------------------------------------------------
...
struct RemoteStorageFileReadAsyncComplete_t {
	...
	SteamAPICall_t
	    m_hFileReadAsync;   // call handle of the async read which was made
	EResult m_eResult;   // result
...
//-----------------------------------------------------------------------------
STEAM_CALLBACK_BEGIN(RemoteStorageLocalFileChange_t,
                     k_iSteamRemoteStorageCallbacks + 33)
