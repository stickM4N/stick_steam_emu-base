...
// through a safety function that checks for interfaces being created or
// destroyed.
//
...
#define STEAM_DEFINE_INTERFACE_ACCESSOR(type, name, expr, kind, version)       \
	...
	STEAM_CLANG_ATTR("interface_accessor_kind:" kind                           \
	                 ";interface_accessor_version:" version ";")               \
	inline type name() {                                                       \
		static void *s_CallbackCounterAndContext[3]                            \
		    = { (void *)&SteamInternal_Init_##name, 0, 0 };                    \
		return *(type *)SteamInternal_ContextInit(                             \
		    s_CallbackCounterAndContext);                                      \
	}

#define STEAM_DEFINE_USER_INTERFACE_ACCESSOR(type, name, version)              \
	STEAM_DEFINE_INTERFACE_ACCESSOR(type, name,                                \
	                                SteamInternal_FindOrCreateUserInterface(   \
	                                    SteamAPI_GetHSteamUser(), version),    \
	                                "user", version)
#define STEAM_DEFINE_GAMESERVER_INTERFACE_ACCESSOR(type, name, version)        \
	STEAM_DEFINE_INTERFACE_ACCESSOR(                                           \
	    type, name,                                                            \
	    SteamInternal_FindOrCreateGameServerInterface(                         \
	        SteamGameServer_GetHSteamUser(), version),                         \
	    "gameserver", version)
...
// Internal functions used by the utility CCallResult objects to receive async
// call results
...
#define _STEAM_CALLBACK_3(extra_code, thisclass, func, param)                  \
	struct CCallbackInternal_##func : private CCallbackImpl<sizeof(param)> {   \
		...
		CCallbackInternal_##func &                                             \
		operator=(const CCallbackInternal_##func &) {                          \
			return *this;                                                      \
		}                                                                      \
                                                                               \
	private:                                                                   \
...
struct CallbackMsg_t {
	...
	int m_iCallback;   // Callback identifier.  (Corresponds to the k_iCallback
	                   // enum in the callback structure.)
...
#	define STEAM_CALLBACK_MEMBER_ARRAY(varidx, vartype, varname, varcount)    \
...
#define STEAM_OUT_ARRAY_COUNT(COUNTER, DESC)                                   \
	STEAM_CLANG_ATTR("out_array_count:" #COUNTER ";desc:" #DESC)
#define STEAM_ARRAY_COUNT(COUNTER) STEAM_CLANG_ATTR("array_count:" #COUNTER ";")
#define STEAM_ARRAY_COUNT_D(COUNTER, DESC)                                     \
	STEAM_CLANG_ATTR("array_count:" #COUNTER ";desc:" #DESC)
#define STEAM_BUFFER_COUNT(COUNTER)                                            \
	STEAM_CLANG_ATTR("buffer_count:" #COUNTER ";")
#define STEAM_OUT_BUFFER_COUNT(COUNTER)                                        \
	STEAM_CLANG_ATTR("out_buffer_count:" #COUNTER ";")
#define STEAM_OUT_STRING_COUNT(COUNTER)                                        \
	STEAM_CLANG_ATTR("out_string_count:" #COUNTER ";")
#define STEAM_DESC(DESC) STEAM_CLANG_ATTR("desc:" #DESC ";")
#define STEAM_CALL_RESULT(RESULT_TYPE)                                         \
	STEAM_CLANG_ATTR("callresult:" #RESULT_TYPE ";")
#define STEAM_CALL_BACK(RESULT_TYPE)                                           \
	STEAM_CLANG_ATTR("callback:" #RESULT_TYPE ";")
...
public:
	...
	inline bool Init();   // NOTE: This is defined in steam_api.h, to avoid this
	                      // file having to include everything
...
public:
	...
	inline bool Init();   // NOTE: This is defined in steam_gameserver.h, to
	                      // avoid this file having to include everything
