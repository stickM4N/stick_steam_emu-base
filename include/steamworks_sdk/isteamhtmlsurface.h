...
public:
	...
	//
	// YOU MUST HAVE IMPLEMENTED HANDLERS FOR HTML_BrowserReady_t,
	// HTML_StartRequest_t, HTML_JSAlert_t, HTML_JSConfirm_t, and
	// HTML_FileOpenDialog_t! See the CALLBACKS section of this interface
	// (AllowStartRequest, etc) for more details. If you do not implement these
	// callback handlers, the browser may appear to hang instead of navigating
	// to new pages or triggering javascript popups.
	//
	...
	// Scale the output display space by this factor, this is useful when
	// displaying content on high dpi devices. Specifies the ratio between
	// physical and logical pixels.
	virtual void SetDPIScalingFactor(HHTMLBrowser unBrowserHandle,
	                                 float flDPIScaling)
	    = 0;
...
STEAM_CALLBACK_MEMBER(0, HHTMLBrowser,
                      unBrowserHandle)   // this browser is now fully created
                                         // and ready to navigate to pages
...
STEAM_CALLBACK_MEMBER(
    1, const char *,
    pBGRA)   // a pointer to the B8G8R8A8 data for this surface, valid until
             // SteamAPI_RunCallbacks is next called
STEAM_CALLBACK_MEMBER(2, uint32,
                      unWide)   // the total width of the pBGRA texture
STEAM_CALLBACK_MEMBER(3, uint32,
                      unTall)   // the total height of the pBGRA texture
STEAM_CALLBACK_MEMBER(
    4, uint32,
...
STEAM_CALLBACK_MEMBER(
    6, uint32, unUpdateWide)   // the width of the damage rect for this update
STEAM_CALLBACK_MEMBER(
    7, uint32, unUpdateTall)   // the height of the damage rect for this update
STEAM_CALLBACK_MEMBER(8, uint32,
                      unScrollX)   // the page scroll the browser was at when
                                   // this texture was rendered
STEAM_CALLBACK_MEMBER(9, uint32,
                      unScrollY)   // the page scroll the browser was at when
                                   // this texture was rendered
STEAM_CALLBACK_MEMBER(
    10, float,
...
STEAM_CALLBACK_MEMBER(1, const char *,
                      pchURL)   // the url they wish to navigate to
STEAM_CALLBACK_MEMBER(2, const char *,
                      pchTarget)   // the html link target type  (i.e _blank,
                                   // _self, _parent, _top )
STEAM_CALLBACK_MEMBER(3, const char *,
                      pchPostData)   // any posted data for the request
STEAM_CALLBACK_MEMBER(4, bool,
                      bIsRedirect)   // true if this was a http/html redirect
...
STEAM_CALLBACK_MEMBER(0, HHTMLBrowser,
                      unBrowserHandle)   // the handle of the surface
STEAM_CALLBACK_END(1)
...
STEAM_CALLBACK_MEMBER(2, const char *,
                      pchPostData)   // any posted data for the request
STEAM_CALLBACK_MEMBER(3, bool,
                      bIsRedirect)   // true if this was a http/html redirect
                                     // from the last load request
STEAM_CALLBACK_MEMBER(4, const char *, pchPageTitle)   // the title of the page
STEAM_CALLBACK_MEMBER(5, bool,
                      bNewNavigation)   // true if this was from a fresh tab and
                                        // not a click on an existing page
...
// forward
//-----------------------------------------------------------------------------
STEAM_CALLBACK_BEGIN(HTML_CanGoBackAndForward_t,
                     k_iSteamHTMLSurfaceCallbacks + 10)
STEAM_CALLBACK_MEMBER(0, HHTMLBrowser,
                      unBrowserHandle)   // the handle of the surface
...
STEAM_CALLBACK_MEMBER(2, uint32,
                      unX)   // the x pos into the page to display the popup
STEAM_CALLBACK_MEMBER(3, uint32,
                      unY)   // the y pos into the page to display the popup
...
//-----------------------------------------------------------------------------
// Purpose: The browser has restarted due to an internal failure, use this new
// handle value
//-----------------------------------------------------------------------------
...
STEAM_CALLBACK_MEMBER(
    1, HHTMLBrowser,
    unOldBrowserHandle)   // the handle for the browser before the restart, if
                          // your handle was this then switch to using
                          // unBrowserHandle for API calls
STEAM_CALLBACK_END(2)
