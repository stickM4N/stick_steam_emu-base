...
public:
	...
	virtual bool
	GetUserAchievementAndUnlockTime(CSteamID steamIDUser, const char *pchName,
	...
	GetLeaderboardDisplayType(SteamLeaderboard_t hSteamLeaderboard)
	    = 0;
	...
	// use a for loop from 0 to LeaderboardScoresDownloaded_t::m_cEntryCount to
	// get all the downloaded entries e.g.
	...
	//				int32 details[3];		// we know this is how many we've
	//stored previously 				GetDownloadedLeaderboardEntry(
	// pLeaderboardScoresDownloaded->m_hSteamLeaderboardEntries, index,
	//&leaderboardEntry, details, 3 ); 				assert(
	//leaderboardEntry.m_cDetails == 3
	//);
	//				...
	//			}
	...
	// regarding how the user got that score pScoreDetails points to an array of
	// int32's, cScoreDetailsCount is the number of int32's in the list
	...
	// leaderboard. hContent is a handle to a piece of user generated content
	// that was shared using ISteamUserRemoteStorage::FileShare(). This call is
	// asynchronous, with the result returned in LeaderboardUGCSet_t.
	STEAM_CALL_RESULT(LeaderboardUGCSet_t)
	virtual SteamAPICall_t
	AttachLeaderboardUGC(SteamLeaderboard_t hSteamLeaderboard, UGCHandle_t hUGC)
	    = 0;
	...
	// Get the info on the most achieved achievement for the game, returns an
	// iterator index you can use to fetch the next most achieved afterwards.
	// Will return -1 if there is no data on achievement percentages (ie, you
	// haven't called RequestGlobalAchievementPercentages and waited on the
	// callback).
	virtual int
	GetMostAchievedAchievementInfo(char *pchName, uint32 unNameBufLen,
	...
	// previous call. Returns -1 after the last achievement has been iterated.
	virtual int
	GetNextMostAchievedAchievementInfo(int iIteratorPrevious, char *pchName,
	...
	// GlobalStatsReceived_t. nHistoryDays specifies how many days of day-by-day
	// history to retrieve in addition to the overall totals. The limit is 60.
	...
	// cubData is the size in bytes of the pubData buffer. Returns the number of
	...
	// For achievements that have related Progress stats, use this to query what
	// the bounds of that progress are. You may want this info to selectively
	// call IndicateAchievementProgress when appropriate milestones of progress
	// have been made, to show a progress notification to the user.
	STEAM_FLAT_NAME(GetAchievementProgressLimitsInt32)
	virtual bool GetAchievementProgressLimits(const char *pchName,
	                                          int32 *pnMinProgress,
	                                          int32 *pnMaxProgress)
	    = 0;
...
struct UserAchievementIconFetched_t {
	...
	int m_nIconHandle;   // Handle to the image, which can be used in
	                     // SteamUtils()->GetImageRGBA(), 0 means no image is
