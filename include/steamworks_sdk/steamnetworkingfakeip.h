...
// code easier.  Although the real MTU might be higher than this, this
// particular conservative value is chosen so that fragmentation won't be
// occurring and hiding performance problems from you.
...
public:
	...
	/// - k_EResultIPNotFound will be returned if the address is a
	/// local/ephemeral
	...
	///   Expect the ping time to fluctuate during this period, and it's
	///   possible that messages will be delivered out of order (which is also
	///   possible with ordinary UDP).
	virtual EResult
	SendMessageToFakeIP(const SteamNetworkingIPAddr &remoteAddress,
	                    const void *pData, uint32 cbData, int nSendFlags)
	    = 0;
	...
	/// nMaxMessages.
	///
	/// SteamNetworkingMessage_t::m_identity in the returned message(s) will
	/// always contain a FakeIP.  See
	/// ISteamNetworkingUtils::GetRealIdentityForFakeIP.
	virtual int ReceiveMessages(SteamNetworkingMessage_t **ppOutMessages,
	                            int nMaxMessages)
	    = 0;

	/// Schedule the internal connection for a given peer to be cleaned up in a
	/// few seconds.
	///
	/// Idle connections automatically time out, and so this is not strictly
	/// *necessary*, but if you have reason to believe that you are done talking
	/// to a given peer for a while, you can call this to speed up the timeout.
	/// If any remaining packets are sent or received from the peer, the cleanup
	/// is canceled and the usual timeout value is restored.  Thus you will
	/// usually call this immediately after sending or receiving
	/// application-layer "close connection" packets.
	virtual void ScheduleCleanup(const SteamNetworkingIPAddr &remoteAddress)
	    = 0;
};
...
struct SteamNetworkingFakeIPResult_t {
	...
	/// - k_EResultBusy - you called GetFakeIP but the request has not
	/// completed.
	/// - k_EResultInvalidParam - you called GetFakeIP with an invalid port
	/// index
