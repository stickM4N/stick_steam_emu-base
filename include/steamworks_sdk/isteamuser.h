...
public:
	...
	//
	// DEPRECATED!  This function will be removed from the SDK in an upcoming
	// version.
	//              Please migrate to BeginAuthSession and related functions.
	virtual int InitiateGameConnection_DEPRECATED(
	...
	// version.
	//              Please migrate to BeginAuthSession and related functions.
	virtual void TerminateGameConnection_DEPRECATED(uint32 unIPServer,
	                                                uint16 usPortServer)
	    = 0;
	...
	// Determine the size of captured audio data that is available from
	// GetVoice. Most applications will only use compressed data and should
	// ignore the other parameters, which exist primarily for backwards
	// compatibility. See comments below for further explanation of
	// "uncompressed" data.
	virtual EVoiceResult
	GetAvailableVoice(uint32 *pcbCompressed,
	                  uint32 *pcbUncompressed_Deprecated = 0,
	...
	// may have been run through preprocessing filters and/or had silence
	// removed, so the uncompressed audio could have a shorter duration than you
	// expect. There may be no data at all during long periods of silence. Also,
	// fetching uncompressed audio will cause GetVoice to discard any leftover
	// compressed audio, so you must fetch both types at once. Finally,
	// GetAvailableVoice is not precisely accurate when the uncompressed size is
	// requested. So if you really need to use uncompressed audio, you should
	// call GetVoice frequently with two very large (20kb+) output buffers
	// instead of trying to allocate perfectly-sized buffers. But most
	// applications should ignore all of these details and simply leave the
	// "uncompressed" parameters as NULL/zero.
	// ---------------------------------------------------------------------------

	// Read captured audio data from the microphone buffer. This should be
	// called at least once per frame, and preferably every few milliseconds, to
	// keep the microphone input delay as low as possible. Most applications
	// will only use compressed data and should pass NULL/zero for the
	// "uncompressed" parameters. Compressed data can be transmitted by your
	// application and decoded into raw using the DecompressVoice function
	// below.
	virtual EVoiceResult
	...
	// Decodes the compressed voice data returned by GetVoice. The output data
	// is raw single-channel 16-bit PCM audio. The decoder supports any sample
	// rate from 11025 to 48000; see GetVoiceOptimalSampleRate() below for
	// details. If the output buffer is not large enough, then *nBytesWritten
	// will be set to the required buffer size, and k_EVoiceResultBufferTooSmall
	// is returned. It is suggested to start with a 20kb buffer and reallocate
	// as necessary.
	virtual EVoiceResult
	DecompressVoice(const void *pCompressed, uint32 cbCompressed,
	...
	// audio device (and/or your application's audio output SDK) deals with
	// lower sample rates. You may find that you get the best audio output
	// quality when you ignore this function and use the native sample rate of
	// your audio output device, which is usually 48000 or 44100.
	...
	// Upon exit, *pcbTicket will be either the size of the ticket copied into
	// your buffer (if true was returned), or the size needed (if false was
	// returned).  To determine the proper size of the ticket, you can pass
	// pTicket=NULL and cbMaxTicket=0; if a ticket is available, *pcbTicket will
	// contain the size needed, otherwise it will be zero.
	...
	// Advise steam china duration control system about the online state of the
	// game. This will prevent offline gameplay time from counting against a
	// user's playtime limits.
	virtual bool
	BSetDurationControlOnlineState(EDurationControlOnlineState eNewState)
	    = 0;
};
...
STEAM_DEFINE_USER_INTERFACE_ACCESSOR(ISteamUser *, SteamUser,
...
//			this means the Steam client now has a working connection to the
//Steam servers 			usually this will have occurred before the game has
// launched, and
// should 			only be seen if the user has dropped connection due to a
...
// message. 			This can usually occur if the user doesn't have rights to play on
// the game server.
//-----------------------------------------------------------------------------
...
//			When getting this message the client should disconnect from Steam,
//reset any stored Steam state and reconnect. 			This usually occurs in
// the rare event the Steam client has some kind of fatal error.
//-----------------------------------------------------------------------------
struct IPCFailure_t {
	enum {
...
struct MarketEligibilityResponse_t {
	...
	int m_cdaySteamGuardRequiredDays;   // The number of days any user is
	                                    // required to have had Steam Guard
	                                    // before they can use the market
	int m_cdayNewDeviceCooldown;   // The number of days after initial device
	                               // authorization a user must wait before
	                               // using the market on that device
};
...
struct DurationControl_t {
	...
	EResult m_eResult;   // result of call (always k_EResultOK for asynchronous
	                     // timer-based notifications)
	AppId_t m_appid;   // appid generating playtime

	bool m_bApplicable;   // is duration control applicable to user + game
	                      // combination
	int32 m_csecsLast5h;   // playtime since most recent 5 hour gap in playtime,
	                       // only counting up to regulatory limit of playtime,
	                       // in seconds

	EDurationControlProgress
	    m_progress;   // recommended progress (either everything is fine, or
	                  // please exit game)
	...
	int32 m_csecsRemaining;   // playtime remaining until the user hits a
	                          // regulatory limit
};
