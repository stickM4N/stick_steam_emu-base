...
enum EP2PSessionError {
	...
	k_EP2PSessionErrorTimeout
	    = 4,   // target isn't responding, perhaps not calling
	           ...
	           // this (NAT traversal is not firewall traversal) make sure that
	           // UDP ports 3478, 4379, and 4380 are open in an outbound
	           // direction

	// The following error codes were removed and will never be sent.
	// For privacy reasons, there is no reply if the user is offline or playing
	// another game.
...
struct P2PSessionState_t {
	...
	uint32 m_nRemoteIP;   // potential IP:Port of remote host. Could be TURN
	                      // server.
...
enum ESNetSocketState {
	...
	k_ESNetSocketStateConnectionBroken
	    = 25,   // connection has been broken; either the other end has
	            // disappeared or our local network connection has broke
};
...
public:
	////////////////////////////////////////////////////////////////////////////////////////////
	//
	// UDP-style (connectionless) networking interface.  These functions send
	// messages using an API organized around the destination.  Reliable and
	// unreliable messages are supported.
	//
	// For a more TCP-style interface (meaning you have a connection handle),
	// see the functions below. Both interface styles can send both reliable and
	// unreliable messages.
	//
	// Automatically establishes NAT-traversing or Relay server connections
	//
	// These APIs are deprecated, and may be removed in a future version of the
	// Steamworks SDK.  See ISteamNetworkingMessages.
	...
	// different systems 	- you'll have to call ReadP2PPacket() with the same
	// channel number in order to retrieve the data on the other end using
	...
	//
	// NOTE: This function is deprecated and may be removed in a future version
	// of the SDK.  For security purposes, we may decide to relay the traffic to
	// certain peers, even if you pass false to this function, to prevent
	// revealing the client's IP address top another peer.
	...
	//
	// These functions are more like a client-server TCP API.  One side is the
	// "server" and "listens" for incoming connections, which then must be
	// "accepted."  The "client" initiates a connection by "connecting." Sending
	// and receiving is done through a connection handle.
	//
	// For a more UDP-style interface, where you do not track connection handles
	// but simply send messages to a SteamID, use the UDP-style functions above.
	//
	// Both methods can send both reliable and unreliable methods.
	//
	// These APIs are deprecated, and may be removed in a future version of the
	// Steamworks SDK.  See ISteamNetworkingSockets.
	//
	////////////////////////////////////////////////////////////////////////////////////////////
	...
	virtual SNetListenSocket_t
	CreateListenSocket(int nVirtualP2PPort, SteamIPAddress_t nIP, uint16 nPort,
	...
	virtual SNetSocket_t
	CreateP2PConnectionSocket(CSteamID steamIDTarget, int nVirtualPort,
	                          int nTimeoutSec, bool bAllowUseOfPacketRelay)
	    = 0;
	virtual SNetSocket_t CreateConnectionSocket(SteamIPAddress_t nIP,
	...
	virtual ESNetSocketConnectionType
...
STEAM_DEFINE_GAMESERVER_INTERFACE_ACCESSOR(ISteamNetworking *,
