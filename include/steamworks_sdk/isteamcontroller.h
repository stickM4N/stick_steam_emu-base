...
//    Note: The older ISteamController interface has been deprecated in favor of
//    ISteamInput - this interface
//			was updated in this SDK but will be removed from future SDK's. The
//Steam Client will retain 			compatibility with the older interfaces so
// your any existing integrations should be unaffected.
//
...
// GetInputTypeForHandle instead Versions of Steam that add new controller types
// in the future will extend this enum if you're using a lookup table please
// check the bounds of any origins returned by Steam.
enum EControllerActionOrigin {
	...
	k_EControllerActionOrigin_Switch_ProGyro_Move,   // Primary Gyro in Pro
	                                                 // Controller, or Right
	                                                 // JoyCon
	k_EControllerActionOrigin_Switch_ProGyro_Pitch,   // Primary Gyro in Pro
	                                                  // Controller, or Right
	                                                  // JoyCon
	k_EControllerActionOrigin_Switch_ProGyro_Yaw,   // Primary Gyro in Pro
	                                                // Controller, or Right
	                                                // JoyCon
	k_EControllerActionOrigin_Switch_ProGyro_Roll,   // Primary Gyro in Pro
	                                                 // Controller, or Right
	                                                 // JoyCon
	// Switch JoyCon Specific
	k_EControllerActionOrigin_Switch_RightGyro_Move,   // Right JoyCon Gyro
	                                                   // generally should
	                                                   // correspond to Pro's
	                                                   // single gyro
	k_EControllerActionOrigin_Switch_RightGyro_Pitch,   // Right JoyCon Gyro
	                                                    // generally should
	                                                    // correspond to Pro's
	                                                    // single gyro
	k_EControllerActionOrigin_Switch_RightGyro_Yaw,   // Right JoyCon Gyro
	                                                  // generally should
	                                                  // correspond to Pro's
	                                                  // single gyro
	k_EControllerActionOrigin_Switch_RightGyro_Roll,   // Right JoyCon Gyro
	                                                   // generally should
	                                                   // correspond to Pro's
	                                                   // single gyro
	...
	k_EControllerActionOrigin_Switch_RightGrip_Lower,   // Right JoyCon SL
	                                                    // Button
	k_EControllerActionOrigin_Switch_RightGrip_Upper,   // Right JoyCon SR
	                                                    // Button
	...
	k_EControllerActionOrigin_Count,   // If Steam has added support for new
...
public:
	...
	// controller state. This must be called from somewhere before
	// GetConnectedControllers will return any handles
	virtual void RunFrame() = 0;

	// Enumerate currently connected controllers
	// handlesOut should point to a STEAM_CONTROLLER_MAX_COUNT sized array of
	// ControllerHandle_t handles Returns the number of handles written to
	// handlesOut
	virtual int GetConnectedControllers(STEAM_OUT_ARRAY_COUNT(
	    STEAM_CONTROLLER_MAX_COUNT, Receives list of connected controllers)
	...
	virtual void ActivateActionSet(ControllerHandle_t controllerHandle,
	    ...
	    = 0;
	virtual ControllerActionSetHandle_t
	GetCurrentActionSet(ControllerHandle_t controllerHandle)
	    = 0;

	// ACTION SET LAYERS
	virtual void
	ActivateActionSetLayer(ControllerHandle_t controllerHandle,
	                       ControllerActionSetHandle_t actionSetLayerHandle)
	    = 0;
	virtual void
	DeactivateActionSetLayer(ControllerHandle_t controllerHandle,
	                         ControllerActionSetHandle_t actionSetLayerHandle)
	    = 0;
	virtual void
	DeactivateAllActionSetLayers(ControllerHandle_t controllerHandle)
	    = 0;
	// Enumerate currently active layers
	// handlesOut should point to a STEAM_CONTROLLER_MAX_ACTIVE_LAYERS sized
	// array of ControllerActionSetHandle_t handles. Returns the number of
	// handles written to handlesOut
	...
	virtual ControllerDigitalActionHandle_t
	...
	// STEAM_CONTROLLER_MAX_ORIGINS sized array of EControllerActionOrigin
	// handles. The EControllerActionOrigin enum will get extended as support
	// for new controller controllers gets added to the Steam client and will
	// exceed the values from this header, please check bounds if you are using
	// a look up table.
	...
	// and store the handles for all future API calls.
	virtual ControllerAnalogActionHandle_t
	...
	// specified origin - this call is serialized
	virtual const char *
	GetStringForActionOrigin(EControllerActionOrigin eOrigin)
	    = 0;

	virtual void
	StopAnalogActionMomentum(ControllerHandle_t controllerHandle,
	                         ControllerAnalogActionHandle_t eAction)
	    = 0;

	// Returns raw motion data from the specified controller
	virtual ControllerMotionData_t
	...
	// Trigger a pulse with a duty cycle of usDurationMicroSec / usOffMicroSec,
	// unRepeat times. nFlags is currently unused and reserved for future use.
	...
	virtual void TriggerVibration(ControllerHandle_t controllerHandle,
	                              unsigned short usLeftSpeed,
	                              unsigned short usRightSpeed)
	    = 0;

	// Set the controller LED color on supported controllers.
	virtual void SetLEDColor(ControllerHandle_t controllerHandle, uint8 nColorR,
	...
	// EControllerActionOrigin which update with Steam and may return
	// unrecognized values ESteamInputType will remain static and only return
	// valid values from your SDK version
	virtual ESteamInputType
	GetInputTypeForHandle(ControllerHandle_t controllerHandle)
	    = 0;
	...
	virtual int
	GetGamepadIndexForController(ControllerHandle_t ulControllerHandle)
	    = 0;
	...
	// the other controller type this will return k_EControllerActionOrigin_None
	virtual EControllerActionOrigin
	...
	virtual bool
	GetControllerBindingRevision(ControllerHandle_t controllerHandle,
