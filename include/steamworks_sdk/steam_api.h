...
//
//	These functions manage loading, initializing and shutdown of the
// steamclient.dll
//
//----------------------------------------------------------------------------------------------------------------------------------------------------------//


// SteamAPI_Init must be called before using any other API functions. If it
// fails, an error message will be output to the debugger (or stderr) with
// further information.
...
// SteamAPI_RestartAppIfNecessary ensures that your executable was launched
// through Steam.
//
// Returns true if the current process should terminate. Steam is now
// re-launching your application.
//
// Returns false if no action needs to be taken. This means that your executable
// was started through the Steam client, or a steam_appid.txt file is present in
// your game's directory (for development). Your current process should continue
// if false is returned.
//
// NOTE: If you use the Steam DRM wrapper on your primary executable file, this
// check is unnecessary since the DRM wrapper will ensure that your application
// was launched properly through Steam.
S_API bool S_CALLTYPE SteamAPI_RestartAppIfNecessary(uint32 unOwnAppID);

// Many Steam API functions allocate a small amount of thread-local memory for
// parameter storage. SteamAPI_ReleaseCurrentThreadMemory() will free API memory
// associated with the calling thread. This function is also called
// automatically by SteamAPI_RunCallbacks(), so a single-threaded program never
// needs to explicitly call this function.
...
// need to load an explicit steam dll by name. DEPRECATED - implementation is
// Windows only, and the path returned is a UTF-8 string which must be converted
// to UTF-16 for use with Win32 APIs
S_API const char *SteamAPI_GetSteamInstallPath();

// sets whether or not Steam_RunCallbacks() should do a try {} catch (...) {}
// around calls to issuing callbacks This is ignored if you are using the manual
...
// pchDate should be of the format "Mmm dd yyyy" (such as from the __ DATE __
// macro ) pchTime should be of the format "hh:mm:ss" (such as from the __ TIME
// __ macro ) bFullMemoryDumps (Win32 only) -- writes out a uuid-full.dmp in the
...
//
// An alternative method for dispatching callbacks.  Similar to a windows
// message loop.
//
...
//
// Here is the basic template for replacing SteamAPI_RunCallbacks() with manual
// dispatch
/*

    HSteamPipe hSteamPipe = SteamAPI_GetHSteamPipe(); // See also
   SteamGameServer_GetHSteamPipe() SteamAPI_ManualDispatch_RunFrame( hSteamPipe
   ) CallbackMsg_t callback; while ( SteamAPI_ManualDispatch_GetNextCallback(
   hSteamPipe, &callback ) )
    {
        // Check for dispatching API call results
        if ( callback.m_iCallback == SteamAPICallCompleted_t::k_iCallback )
        {
            SteamAPICallCompleted_t *pCallCompleted = (SteamAPICallCompleted_t
   *)callback. void *pTmpCallResult = malloc( pCallback->m_cubParam ); bool
   bFailed; if ( SteamAPI_ManualDispatch_GetAPICallResult( hSteamPipe,
   pCallCompleted->m_hAsyncCall, pTmpCallResult, pCallback->m_cubParam,
...
/// Inform the API that you wish to use manual event dispatch.  This must be
/// called after SteamAPI_Init, but before you use any of the other manual
...
/// Fetch the next pending callback on the given pipe, if any.  If a callback is
/// available, true is returned and the structure is populated.  In this case,
/// you MUST call SteamAPI_ManualDispatch_FreeLastCallback (after dispatching
/// the callback) before calling SteamAPI_ManualDispatch_GetNextCallback again.
S_API bool S_CALLTYPE SteamAPI_ManualDispatch_GetNextCallback(
    HSteamPipe hSteamPipe, CallbackMsg_t *pCallbackMsg);

/// You must call this after dispatching the callback, if
/// SteamAPI_ManualDispatch_GetNextCallback returns true.
S_API void S_CALLTYPE
SteamAPI_ManualDispatch_FreeLastCallback(HSteamPipe hSteamPipe);

/// Return the call result for the specified call on the specified pipe.  You
/// really should only call this in a handler for SteamAPICallCompleted_t
/// callback.
...
// CSteamAPIContext
//
// Deprecated!  This is not necessary any more.  Please use the global accessors
// directly
//
//----------------------------------------------------------------------------------------------------------------------------------------------------------//
...
#	if !defined(ANDROID)                                                      \
	    && !defined(IOSALL)   // Not yet supported on Android or ios.
