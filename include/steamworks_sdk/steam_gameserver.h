...
enum EServerMode {
	...
	eServerModeNoAuthentication = 1,   // Don't authenticate user logins and
	                                   // don't list on the server list
...
/// Pass to SteamGameServer_Init to indicate that the same UDP port will be used
/// for game traffic UDP queries for server browser pings and LAN discovery.  In
/// this case, Steam will not open up a socket to handle server browser queries,
/// and you must use ISteamGameServer::HandleIncomingPacket and
/// ISteamGameServer::GetNextOutgoingPacket to handle packets related to server
/// discovery on your socket.
const uint16 STEAMGAMESERVER_QUERY_PORT_SHARED = 0xffff;

// DEPRECATED: This old name was really confusing.
const uint16 MASTERSERVERUPDATERPORT_USEGAMESOCKETSHARE
...
// addresses, you can pass a non-zero
//   value here and the relevant sockets will be bound to that IP.  This can be
//   used to ensure that the IP you desire is the one used in the server
//   browser.
// - usGamePort is the port that clients will connect to for gameplay.  You will
// usually open up your
//   own socket bound to this port.
// - usQueryPort is the port that will manage server browser related duties and
// info
//		pings from clients.  If you pass STEAMGAMESERVER_QUERY_PORT_SHARED for
// usQueryPort, then it 		will use "GameSocketShare" mode, which means that
// the game is responsible for sending and receiving 		UDP packets for the
// master server updater.  (See ISteamGameServer::HandleIncomingPacket and
//		ISteamGameServer::GetNextOutgoingPacket.)
// - The version string should be in the form x.x.x.x, and is used by the master
// server to detect when the
//		server is out of date.  (Only servers with the latest version will be
// listed.)
inline bool SteamGameServer_Init(uint32 unIP, uint16 usGamePort,
...
inline bool CSteamGameServerAPIContext::Init() {
	...
	if (!m_pSteamGameServer || !m_pSteamGameServerUtils
...
inline bool SteamGameServer_Init(uint32 unIP, uint16 usGamePort,
                                 uint16 usQueryPort, EServerMode eServerMode,
                                 const char *pchVersionString) {
	if (!SteamInternal_GameServer_Init(unIP, 0, usGamePort, usQueryPort,
