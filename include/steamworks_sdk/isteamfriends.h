...
enum EFriendRelationship {
	...
	k_EFriendRelationshipBlocked
	    = 1,   // this doesn't get stored; the user has just done an Ignore on
	           // an friendship invite
	...
	k_EFriendRelationshipSuggested_DEPRECATED
	    = 7,   // was used by the original implementation of the facebook
	           // linking feature, but now unused.
...
enum EPersonaState {
	...
	k_EPersonaStateInvisible
	    = 7,   // Online, but appears offline to friends.  This status is never
...
enum EActivateGameOverlayToWebPageMode {
	k_EActivateGameOverlayToWebPageMode_Default
	    = 0,   // Browser will open next to all other windows that the user has
	           // open in the overlay. The window will remain open, even if the
	           // user closes then re-opens the overlay.

	k_EActivateGameOverlayToWebPageMode_Modal
	    = 1   // Browser will be opened in a special overlay configuration which
	          // hides all other windows that the user has open in the overlay.
	          // When the user closes the overlay, the browser window will also
	          // close. When the user closes the browser window, the overlay
...
//			that can be a friend, in a group, on a game server or in a lobby
//with the local user
//-----------------------------------------------------------------------------
class ISteamFriends {
public:
	...
	// Returns NULL if the no nickname has been set for that player. DEPRECATED:
	// GetPersonaName follows the Steam nickname preferences, so apps shouldn't
	// need to care about nicknames explicitly.
	...
	DownloadClanActivityCounts(STEAM_ARRAY_COUNT(cClansToRequest)
	...
	// valid options include "Friends", "Community", "Players", "Settings",
	...
	// groups profile 		"chat" - opens a chat window to the specified user,
	// or joins the group chat 		"jointrade" - opens a window to a Steam
	// Trading session that was started with the ISteamEconomy/StartTrade Web
	// API 		"stats"
	//- opens the overlay web browser to the specified user's stats
	//		"achievements" - opens the overlay web browser to the specified
	//user's achievements 		"friendadd" - opens the overlay in minimal mode
	// prompting the user to add the target user as a friend
	// "friendremove" - opens the overlay in minimal mode prompting the user to
	// remove the target friend 		"friendrequestaccept" - opens the overlay in
	//minimal mode prompting the user to accept an incoming friend invite
	// "friendrequestignore" - opens the overlay in minimal mode prompting the
	// user to ignore an incoming friend invite
	...
	// same game Each user has a set of Key/Value pairs Note the following
	...
	// dialog in the Steam friends list 		"connect" - a UTF-8 string that
	// contains the command-line for how a friend can connect to a game
	// "steam_display"
	//- Names a rich presence localization token that will be displayed in the
	// viewing user's selected language 									  in the Steam client UI. For
	// more info:
	// https://partner.steamgames.com/doc/api/ISteamFriends#richpresencelocalization
	//		"steam_player_group"		- When set, indicates to the Steam client
	//that the player is a member of a particular group. Players in the same
	// group 									  may be organized together in various places in the Steam UI.
	// "steam_player_group_size"	- When set, indicates the total number of
	// players in the steam_player_group. The Steam client may use this number
	// to 									  display additional information about a group when all of
	// the members are not part of a user's friends list.
	...
	// Rich invite support.
	// If the target accepts the invite, a GameRichPresenceJoinRequested_t
	// callback is posted containing the connect string. (Or you can configure
	// your game so that it is passed on the command line instead.  This is a
	// deprecated path; ask us if you really need this.)
	...
	///
	/// You can register for UnreadChatMessagesChanged_t callbacks to know when
	/// this has potentially changed.
	virtual int GetNumChatsWithUnreadPriorityMessages() = 0;

	// activates game overlay to open the remote play together invite dialog.
	// Invitations will be sent for remote play together
	virtual void
	ActivateGameOverlayRemotePlayTogetherInviteDialog(CSteamID steamIDLobby)
	    = 0;

	// Call this before calling ActivateGameOverlayToWebPage() to have the Steam
	// Overlay Browser block navigations to your specified protocol (scheme)
	// uris and instead dispatch a OverlayBrowserProtocolNavigation_t callback
	// to your game. ActivateGameOverlayToWebPage() must have been called with
	// k_EActivateGameOverlayToWebPageMode_Modal
	virtual bool RegisterProtocolInOverlayBrowser(const char *pchProtocol) = 0;

	// Activates the game overlay to open an invite dialog that will send the
	// provided Rich Presence connect string to selected friends
	virtual void
	ActivateGameOverlayInviteDialogConnectString(const char *pchConnectString)
	    = 0;
};
...
// their friends list
//			game client should attempt to connect to specified server when this
//is received
//-----------------------------------------------------------------------------
...
//			game client should attempt to connect to specified lobby when this
//is received
//-----------------------------------------------------------------------------
...
struct SetPersonaNameResponse_t {
	...
	bool m_bLocalSuccess;   // true if name change was retained locally.  (We
	                        // might not have been able to communicate with
	                        // Steam)
...
//-----------------------------------------------------------------------------
// Purpose: Dispatched when an overlay browser instance is navigated to a
