...
//#define STEAMNETWORKINGSOCKETS_STANDALONELIB // Comment this in to support
// compiling/linking with the standalone library / gamenetworkingsockets
// opensource
#define STEAMNETWORKINGSOCKETS_STEAMAPI   // Compiling/link with steam_api.h and
                                          // Steamworks SDK
//-----------------------------------------------------------------------------

#if !defined(STEAMNETWORKINGSOCKETS_OPENSOURCE)                                \
    && !defined(STEAMNETWORKINGSOCKETS_STREAMINGCLIENT)
...
typedef void (*FnSteamNetConnectionStatusChanged)(
...
typedef void (*FnSteamNetworkingMessagesSessionRequest)(
    SteamNetworkingMessagesSessionRequest_t *);
typedef void (*FnSteamNetworkingMessagesSessionFailed)(
...
/// Identifier used for a network location point of presence.  (E.g. a Valve
/// data center.) Typically you won't need to directly manipulate these.
typedef uint32 SteamNetworkingPOPID;

/// A local timestamp.  You can subtract two timestamps to get the number of
/// elapsed microseconds.  This is guaranteed to increase over time during the
/// lifetime of a process, but not globally across runs.  You don't need to
/// worry about the value wrapping around.  Note that the underlying clock might
/// not actually have microsecond resolution.
...
enum ESteamNetworkingAvailability {
	...
	//
	// In general, we will not automatically retry unless you take some action
	// that depends on of requests this resource, such as querying the status,
	// attempting to initiate a connection, receive a connection, etc.  If you
	// do not take any action at all, we do not automatically retry in the
	// background.
	k_ESteamNetworkingAvailability_CannotTry
	    = -102,   // A dependent resource is missing, so this service is
	              // unavailable.  (E.g. we cannot talk to routers because
	              // Internet is down or we don't have the network config.)
	k_ESteamNetworkingAvailability_Failed
	    = -101,   // We have tried for enough time that we would expect to have
	              // been successful by now.  We have never been successful
	k_ESteamNetworkingAvailability_Previously
	    = -100,   // We tried and were successful at one time, but now it looks
	              // like we have a problem

	k_ESteamNetworkingAvailability_Retrying
	    = -10,   // We previously failed and are currently retrying

	// Not a problem, but not ready either
	k_ESteamNetworkingAvailability_NeverTried
	    = 1,   // We don't know because we haven't ever checked/tried
	k_ESteamNetworkingAvailability_Waiting
	    = 2,   // We're waiting on a dependent resource to be acquired.  (E.g.
	           // we cannot obtain a cert until we are logged into Steam.  We
	           // cannot measure latency to relays until we have the network
	           // config.)
	k_ESteamNetworkingAvailability_Attempting
	    = 3,   // We're actively trying now, but are not yet successful.

	k_ESteamNetworkingAvailability_Current
	    = 100,   // Resource is online/available


	k_ESteamNetworkingAvailability_Unknown
	    = 0,   // Internal dummy/sentinel, or value is not applicable in this
	           // context
...
enum ESteamNetworkingFakeIPType {
	k_ESteamNetworkingFakeIPType_Invalid,   // Error, argument was not even an
	                                        // IP address, etc.
	k_ESteamNetworkingFakeIPType_NotFake,   // Argument was a valid IP, but was
	                                        // not from the reserved "fake"
	                                        // range
	k_ESteamNetworkingFakeIPType_GlobalIPv4,   // Globally unique (for a given
	                                           // app) IPv4 address.  Address
	                                           // space managed by Steam
	k_ESteamNetworkingFakeIPType_LocalIPv4,   // Locally unique IPv4 address.
	                                          // Address space managed by the
	                                          // local process.  For internal
	                                          // use only; should not be shared!
...
struct SteamNetworkingIPAddr {
	...
	bool IsIPv6AllZeros()
	    const;   // Return true if the IP is ::0.  (Doesn't check port.)
	void
	SetIPv6(const uint8 *ipv6,
	        uint16 nPort);   // Set IPv6 address.  IP is interpreted as bytes,
	                         // so there are no endian issues.  (Same as
	                         // inaddr_in6.) The IP can be a mapped IPv4 address
	void SetIPv4(uint32 nIP,
	             uint16 nPort);   // Sets to IPv4 mapped address.  IP and port
	                              // are in host byte order.
	bool IsIPv4() const;   // Return true if IP is mapped IPv4
	uint32
	GetIPv4() const;   // Returns IP in host byte order (e.g. aa.bb.cc.dd as
	                   // 0xaabbccdd).  Returns 0 if IP is not mapped IPv4.
	void SetIPv6LocalHost(uint16 nPort
	                      = 0);   // Set to the IPv6 localhost address ::1, and
	                              // the specified port.
	bool IsLocalHost() const;   // Return true if this identity is localhost.
	                            // (Either IPv6 ::1, or IPv4 127.0.0.1)

	// Max length of the buffer needed to hold IP formatted using ToString,
	// including '\0'
	...
	/// canonical form according to RFC5952.  If you include the port, IPv6 will
	/// be surrounded by brackets, e.g. [::1:2]:80.  Your buffer should be at
	/// least k_cchMaxString bytes to avoid truncation
	///
	/// See also SteamNetworkingIdentityRender
	inline void ToString(char *buf, size_t cbBuf, bool bWithPort) const;

	/// Parse an IP address and optional port.  If a port is not present, it is
	/// set to 0. (This means that you cannot tell if a zero port was explicitly
	/// specified.)
...
/// An abstract way to represent the identity of a network host.  All identities
/// can be represented as simple string.  Furthermore, this string
/// representation is actually used on the wire in several places, even though
/// it is less efficient, in order to facilitate forward compatibility.  (Old
/// client code can handle an identity type that it doesn't understand.)
struct SteamNetworkingIdentity {
	...
	void Clear();
	bool IsInvalid()
	    const;   // Return true if we are the invalid type.  Does not make any
	             // other validity checks (e.g. is SteamID actually valid)

	void SetSteamID(CSteamID steamID);
	CSteamID GetSteamID() const;   // Return black CSteamID (!IsValid()) if
	                               // identity is not a SteamID
	...
	void
	SetIPAddr(const SteamNetworkingIPAddr &addr);   // Set to specified IP:port
	const SteamNetworkingIPAddr *
	GetIPAddr() const;   // returns null if we are not an IP address.
	void SetIPv4Addr(uint32 nIPv4,
	                 uint16 nPort);   // Set to specified IPv4:port
	...
	void SetLocalHost();   // Set to localhost.  (We always use IPv6 ::1 for
	                       // this, not 127.0.0.1)
	bool IsLocalHost() const;   // Return true if this identity is localhost.

	bool SetGenericString(
	    const char *pszString);   // Returns false if invalid length
	const char *
	GetGenericString() const;   // Returns nullptr if not generic string type

	bool SetGenericBytes(const void *data,
	                     size_t cbLen);   // Returns false if invalid size.
	const uint8 *GetGenericBytes(
	    int &cbLen) const;   // Returns null if not generic bytes type
	...
	/// Parse back a string that was generated using ToString.  If we don't
	/// understand the string, but it looks "reasonable" (it matches the pattern
	/// type:<type-data> and doesn't have any funky characters, etc), then we
	/// will return true, and the type is set to
	/// k_ESteamNetworkingIdentityType_UnknownType.  false will only be returned
	/// if the string looks invalid.
	bool ParseString(const char *pszStr);

	// Max sizes
	enum {
		k_cchMaxString
		    = 128,   // Max length of the buffer needed to hold any identity,
		             // formatted in string format by ToString
		k_cchMaxGenericString
		    = 32,   // Max length of the string for generic string identities.
	...
	union {
		...
		uint32 m_reserved[32];   // Pad structure to leave easy room for future
		                         // expansion
	};
};
#pragma pack(pop)
...
enum ESteamNetworkingConnectionState {
	...
	/// handle to free up resources.  If there are any messages in the inbound
	/// queue, you may retrieve them.  Otherwise, nothing may be done with the
	/// connection except to close it.
	///
	/// This stats is similar to CLOSE_WAIT in the TCP state machine.
	k_ESteamNetworkingConnectionState_ClosedByPeer = 4,

	/// A disruption in the connection has been detected locally.  (E.g.
	/// timeout, local internet connection disrupted, etc.)
	///
	/// The connection still exists from an API perspective.  You must close the
	...
	//
	// The following values are used internally and will not be returned by any
	// API. We document them here to provide a little insight into the state
	// machine that is used under the hood.
	//

	/// We've disconnected on our side, and from an API perspective the
	/// connection is closed. No more data may be sent or received.  All
	/// reliable data has been flushed, or else we've given up and discarded it.
	/// We do not yet know for sure that the peer knows the connection has been
	/// closed, however, so we're just hanging around so that if we do get a
	/// packet from them, we can send them the appropriate packets so that they
	/// can know why the connection was closed (and not have to rely on a
	/// timeout, which makes it appear as if something is wrong).
	k_ESteamNetworkingConnectionState_FinWait = -1,

	/// We've disconnected on our side, and from an API perspective the
	/// connection is closed. No more data may be sent or received.  From a
	/// network perspective, however, on the wire, we have not yet given any
	/// indication to the peer that the connection is closed. We are in the
	/// process of flushing out the last bit of reliable data.  Once that is
	/// done, we will inform the peer that the connection has been closed, and
	/// transition to the FinWait state.
	///
	/// Note that no indication is given to the remote host that we have closed
	/// the connection, until the data has been flushed.  If the remote host
	/// attempts to send us data, we will do whatever is necessary to keep the
	/// connection alive until it can be closed properly. But in fact the data
	/// will be discarded, since there is no way for the application to read it
	/// back.  Typically this is not a problem, as application protocols that
	/// utilize the lingering functionality are designed for the remote host to
	/// wait for the response before sending any more data.
...
/// Enumerate various causes of connection termination.  These are designed to
/// work similar to HTTP error codes: the numeric range gives you a rough
/// classification as to the source of the problem.
enum ESteamNetConnectionEnd {
	...
	// you want to plumb through application-specific reason codes.  If you
	// don't need this facility, feel free to always pass
	...
	// illegal to pass a code in this range to
	...
	// * We have evidence of recent failures to communicate with multiple
	// relays.
...
const int k_nSteamNetworkConnectionInfoFlags_Unauthenticated
    = 1;   // We don't have a certificate for the remote host.
const int k_nSteamNetworkConnectionInfoFlags_Unencrypted
    = 2;   // Information is being sent out over a wire unencrypted (by this
           // library)
const int k_nSteamNetworkConnectionInfoFlags_LoopbackBuffers
    = 4;   // Internal loopback buffers.  Won't be true for localhost.  (You can
           // check the address to determine that.)  This implies
           // k_nSteamNetworkConnectionInfoFlags_FastLAN
const int k_nSteamNetworkConnectionInfoFlags_Fast
    = 8;   // The connection is "fast" and "reliable".  Either
           // internal/localhost (check the address to find out), or the peer is
           // on the same LAN.  (Probably.  It's based on the address and the
           // ping time, this is actually hard to determine unambiguously).
const int k_nSteamNetworkConnectionInfoFlags_Relayed
    = 16;   // The connection is relayed somehow (SDR or TURN).
const int k_nSteamNetworkConnectionInfoFlags_DualWifi
    = 32;   // We're taking advantage of dual-wifi multi-path

/// Describe the state of a connection.
struct SteamNetConnectionInfo_t {

	/// Who is on the other end?  Depending on the connection type and phase of
	/// the connection, we might not know
	...
	/// Remote address.  Might be all 0's if we don't know it, or if this is
	/// N/A. (E.g. Basically everything except direct UDP connection.)
	...
	char
	    m_szConnectionDescription[k_cchSteamNetworkingMaxConnectionDescription];
...
struct SteamNetConnectionRealTimeStatus_t {
	...
	/// Connection quality measured locally, 0...1.  (Percentage of packets
	/// delivered end-to-end in order).
	...
	/// reliable number ALSO includes data that was previously placed on the
	/// wire, but has now been scheduled for re-transmission.  Thus, it's
	/// possible to observe m_cbPendingReliable increasing between two checks,
	/// even if no calls were made to send reliable data between the checks.
	/// Data that is awaiting the Nagle delay will appear in these numbers.
	...
	/// queued and put on the wire at a metered rate.  Even sending a small
	/// amount of data (e.g. a few MTU, say ~3k) will require some of the data
	/// to be delayed a bit.
	///
	/// Ignoring multiple lanes, the estimated delay will be approximately equal
	/// to
	///
	///		( m_cbPendingUnreliable+m_cbPendingReliable ) /
	/// m_nSendRateBytesPerSecond
	///
	/// plus or minus one MTU.  It depends on how much time has elapsed since
	/// the last packet was put on the wire.  For example, the queue might have
	/// *just* been emptied, and the last packet placed on the wire, and we are
	/// exactly up against the send rate limit.  In that case we might need to
	/// wait for one packet's worth of time to elapse before we can send again.
	/// On the other extreme, the queue might have data in it waiting for Nagle.
	/// (This will always be less than one packet, because as soon as we have a
	/// complete packet we would send it.)  In that case, we might be ready to
	/// send data now, and this value will be 0.
	///
	/// This value is only valid if multiple lanes are not used.  If multiple
	/// lanes are in use, then the queue time will be different for each lane,
	/// and you must use the value in SteamNetConnectionRealTimeLaneStatus_t.
	///
...
struct SteamNetConnectionRealTimeLaneStatus_t {
	...
	/// Lane-specific queue time.  This value takes into consideration lane
	/// priorities and weights, and how much data is queued in each lane, and
	/// attempts to predict how any data currently queued will be sent out.
	SteamNetworkingMicroseconds m_usecQueueTime;
...
struct SteamNetworkingMessage_t {
	...
	/// For messages received on connections: what connection did this come
	/// from? For outgoing messages: what connection to send it to? Not used
	/// when using the ISteamNetworkingMessages interface
	...
	/// it's zero, release the message.  You should not set this function
	/// pointer, or need to access this directly!  Use the Release() function
	/// instead!
	...
	/// valid. For outbound messages, all bits are relevant
	int m_nFlags;
	...
	/// m_pfnFreeData might use this field.)
	///
	/// Not used for received messages.
	int64 m_nUserData;

	/// For outbound messages, which lane to use?  See
	/// ISteamNetworkingSockets::ConfigureConnectionLanes. For inbound messages,
	/// what lane was the message received on?
	uint16 m_idxLane;
	uint16 _pad1__;
...
protected:
	...
	// - You will receive a pointer to a message object when you receive
	// messages (e.g. ISteamNetworkingSockets::ReceiveMessagesOnConnection)
	// - You can allocate a message object for efficient sending using
...
// By default, Nagle's algorithm is applied to all outbound messages.  This
// means that the message will NOT be sent immediately, in case further messages
// are sent soon after you send this, which can be grouped together.  Any time
// there is enough buffered data to fill a packet, the packets will be pushed
// out immediately, but partially-full packets not be sent until the Nagle timer
...
// useful if they are excessively delayed, such as voice data. NOTE: The Nagle
// algorithm is not used, and if the message is not dropped, any messages
// waiting on the Nagle timer are immediately flushed.
//
...
// k_ESteamNetworkingSendType_Unreliable for more details. See
...
// By default, message sending is queued, and the work of encryption and talking
// to the operating system sockets, etc is done on a service thread.  This is
// usually a a performance win when messages are sent from the "main thread".
// However, if this flag is set, and data is ready to be sent immediately
// (either from this message or earlier queued data), then that work will be
// done in the current thread, before the current call returns.  If data is not
// ready to be sent (due to rate limiting or Nagle), then this flag has no
// effect.
//
// This is an advanced flag used to control performance at a very low level. For
// most applications running on modern hardware with more than one CPU core,
// doing the work of sending on a service thread will yield the best
// performance.  Only use this flag if you have a really good reason and
// understand what you are doing. Otherwise you will probably just make
// performance worse.
const int k_nSteamNetworkingSend_UseCurrentThread = 16;

// When sending a message using ISteamNetworkingMessages, automatically
// re-establish a broken session, without returning k_EResultNoConnection.
// Without this flag, if you attempt to send a message, and the session was
// proactively closed by the peer, or an error occurred that disrupted
// communications, then you must close the session using
// ISteamNetworkingMessages::CloseSessionWithUser before attempting to send
// another message.  (Or you can simply add this flag and retry.)  In this way,
// the disruption cannot go unnoticed, and a more clear order of events can be
// ascertained. This is especially important when reliable messages are used,
// since if the connection is disrupted, some of those messages will not have
// been delivered, and it is in general not possible to know which.  Although a
// SteamNetworkingMessagesSessionFailed_t callback will be posted when an error
// occurs to notify you that a failure has happened, callbacks are asynchronous,
// so it is not possible to tell exactly when it happened.  And because the
// primary purpose of ISteamNetworkingMessages is to be like UDP, there is no
// notification when a peer closes the session.
//
// If you are not using any reliable messages (e.g. you are using
// ISteamNetworkingMessages exactly as a transport replacement for UDP-style
// datagrams only), you may not need to know when an underlying connection
...
///
/// NOTE: This object should only be used in the same process!  Do not serialize
/// it, send it over the wire, or persist it in a file or database!  If you need
...
enum ESteamNetworkingConfigScope {
	...
	/// Get/set global option, or defaults.  Even options that apply to more
	/// specific scopes have global scope, and you may be able to just change
	/// the global defaults.  If you need different settings per connection (for
	/// example), then you will need to set those options at the more specific
	/// scope.
	k_ESteamNetworkingConfig_Global = 1,

	/// Some options are specific to a particular interface.  Note that all
	/// connection and listen socket settings can also be set at the interface
	/// level, and they will apply to objects created through those interfaces.
	k_ESteamNetworkingConfig_SocketsInterface = 2,

	/// Options for a listen socket.  Listen socket options can be set at the
	/// interface layer, if  you have multiple listen sockets and they all use
	/// the same options. You can also set connection options on a listen
	/// socket, and they set the defaults for all connections accepted through
	/// this listen socket.  (They will be used if you don't set a connection
	/// option.)
...
enum ESteamNetworkingConfigValue {
	...
	/// a config value, instead of using
	/// ISteamNetworkingSockets::SetConnectionUserData in two specific
	/// instances:
	///
	...
	/// [connection int32] Nagle time, in microseconds.  When SendMessage is
	/// called, if the outgoing message is less than the size of the MTU, it
	/// will be queued for a delay equal to the Nagle timer value.  This is to
	/// ensure that if the application sends several small messages rapidly,
	/// they are coalesced into a single packet. See historical RFC 896.  Value
	/// is in microseconds. Default is 5000us (5ms).
	k_ESteamNetworkingConfig_NagleTime = 12,

	/// [connection int32] Don't automatically fail IP connections that don't
	/// have strong auth.  On clients, this means we will attempt the connection
	/// even if we don't know our identity or can't get a cert.  On the server,
	/// it means that we won't automatically reject a connection due to a
	/// failure to authenticate. (You can examine the incoming connection and
	/// decide whether to accept it.)
	///
	/// This is a dev configuration value, and you should not let users modify
	/// it in production.
	k_ESteamNetworkingConfig_IP_AllowWithoutAuth = 23,

	/// [connection int32] Do not send UDP packets with a payload of
	/// larger than N bytes.  If you set this,
	/// k_ESteamNetworkingConfig_MTU_DataSize is automatically adjusted
	...
	/// communication. 0: Not allowed (the default) 1: Allowed, but prefer
	/// encrypted 2: Allowed, and preferred 3: Required.  (Fail the connection
	/// if the peer requires encryption.)
	///
	/// This is a dev configuration value, since its purpose is to disable
	/// encryption. You should not let users modify it in production.  (But note
	/// that it requires the peer to also modify their value in order for
	/// encryption to be disabled.)
	k_ESteamNetworkingConfig_Unencrypted = 34,

	/// [connection int32] Set this to 1 on outbound connections and listen
	/// sockets, to enable "symmetric connect mode", which is useful in the
	/// following common peer-to-peer use case:
	///
	/// - The two peers are "equal" to each other.  (Neither is clearly the
	/// "client"
	///   or "server".)
	...
	///   peers initiate connections simultaneously, a protocol is needed for
	///   them to resolve the conflict, so that we end up with a single
	///   connection.
	///
	/// This use case is both common, and involves subtle race conditions and
	/// tricky pitfalls, which is why the API has support for dealing with it.
	///
	/// If an incoming connection arrives on a listen socket or via custom
	/// signaling, and the application has not attempted to make a matching
	/// outbound connection in symmetric mode, then the incoming connection can
	/// be accepted as usual. A "matching" connection means that the relevant
	/// endpoint information matches. (At the time this comment is being
	/// written, this is only supported for P2P connections, which means that
	/// the peer identities must match, and the virtual port must match.  At a
	/// later time, symmetric mode may be supported for other connection types.)
	///
	/// If connections are initiated by both peers simultaneously, race
	/// conditions can arise, but fortunately, most of them are handled
	/// internally and do not require any special awareness from the
	/// application.  However, there is one important case that application code
	/// must be aware of: If application code attempts an outbound connection
	/// using a ConnectXxx function in symmetric mode, and a matching incoming
	/// connection is already waiting on a listen socket, then instead of
	/// forming a new connection, the ConnectXxx call will accept the existing
	/// incoming connection, and return a connection handle to this accepted
	/// connection. IMPORTANT: in this case, a
	/// SteamNetConnectionStatusChangedCallback_t has probably *already* been
	/// posted to the queue for the incoming connection! (Once callbacks are
	/// posted to the queue, they are not modified.)  It doesn't matter if the
	/// callback has not been consumed by the app.  Thus, application code that
	/// makes use of symmetric connections must be aware that, when processing a
	/// SteamNetConnectionStatusChangedCallback_t for an incoming connection,
	/// the m_hConn may refer to a new connection that the app has has not seen
	/// before (the usual case), but it may also refer to a connection that has
	/// already been accepted implicitly through a call to Connect()!  In this
	...
	/// may exist at any given time.  If client code attempts to create a
	/// connection, and a (live) connection already exists on the local host,
	/// then either the existing connection will be accepted as described above,
	/// or the attempt to create a new connection will fail.  Furthermore,
	/// linger mode functionality is not supported on symmetric connections.
	///
	/// A more complicated race condition can arise if both peers initiate a
	/// connection at roughly the same time.  In this situation, each peer will
	/// receive an incoming connection from the other peer, when the application
	/// code has already initiated an outgoing connection to that peer.  The
	/// peers must resolve this conflict and decide who is going to act as the
	/// "server" and who will act as the "client". Typically the application
	/// does not need to be aware of this case as it is handled internally.  On
	/// both sides, the will observe their outbound connection being "accepted",
	/// although one of them one have been converted internally to act as the
	/// "server".
	///
	/// In general, symmetric mode should be all-or-nothing: do not mix
	/// symmetric connections with a non-symmetric connection that it might
	/// possible "match" with.  If you use symmetric mode on any connections,
	/// then both peers should use it on all connections, and the corresponding
	/// listen socket, if any.  The behaviour when symmetric and ordinary
	/// connections are mixed is not defined by this API, and you should not
	/// rely on it.  (This advice only applies when connections might possibly
	/// "match".  For example, it's OK to use all symmetric mode connections on
	/// one virtual port, and all ordinary, non-symmetric connections on a
	/// different virtual port, as there is no potential for ambiguity.)
	///
	...
	/// - When creating a listen socket.  (Note that this will automatically
	/// cause
	///   any accepted connections to inherit the flag.)
	/// - When using custom signaling, before accepting an incoming connection.
	///
	/// Setting the flag on listen socket and accepted connections will enable
	/// the API to automatically deal with duplicate incoming connections, even
	/// if the local host has not made any outbound requests.  (In general, such
	/// duplicate requests from a peer are ignored internally and will not be
	/// visible to the application code.  The previous connection must be closed
	/// or resolved first.)
	k_ESteamNetworkingConfig_SymmetricConnect = 37,

	/// [connection int32] For connection types that use "virtual ports", this
	/// can be used to assign a local virtual port.  For incoming connections,
	/// this will always be the virtual port of the listen socket (or the port
	/// requested by the remote host if custom signaling is used and the
	/// connection is accepted), and cannot be changed.  For connections
	/// initiated locally, the local virtual port will default to the same as
	/// the requested remote virtual port, if you do not specify a different
	/// option when creating the connection.  The local port is only relevant
	/// for symmetric connections, when determining if two connections "match."
	/// In this case, if you need the local and remote port to differ, you can
	/// set this value.
	///
	...
	/// 0 = no, 1 = yes, 2 = simulate it for debugging, even if dual wifi not
	/// available
	...
	/// [global float 0--100] Globally duplicate some percentage of packets we
	/// send
	...
	// On Steam, you may use the default Steam callback dispatch mechanism.  If
	// you prefer to not use this dispatch mechanism (or you are not running
	// with Steam), or you want to associate specific functions with specific
	// listen sockets or connections, you can register them as configuration
	// values.
	//
	// Note also that ISteamNetworkingUtils has some helpers to set these
	// globally.

	/// [connection FnSteamNetConnectionStatusChanged] Callback that will be
	/// invoked when the state of a connection changes.
	///
	/// IMPORTANT: callbacks are dispatched to the handler that is in effect at
	/// the time the event occurs, which might be in another thread.  For
	/// example, immediately after creating a listen socket, you may receive an
	/// incoming connection.  And then immediately after this, the remote host
	/// may close the connection.  All of this could happen before the function
	/// to create the listen socket has returned.  For this reason, callbacks
	/// usually must be in effect at the time of object creation.  This means
	/// you should set them when you are creating the listen socket or
	/// connection, or have them in effect so they will be inherited at the time
	/// of object creation.
	///
	/// For example:
	///
	/// exterm void MyStatusChangedFunc(
	/// SteamNetConnectionStatusChangedCallback_t *info );
	/// SteamNetworkingConfigValue_t opt; opt.SetPtr(
	/// k_ESteamNetworkingConfig_Callback_ConnectionStatusChanged,
	/// MyStatusChangedFunc ); SteamNetworkingIPAddr localAddress;
	/// localAddress.Clear(); HSteamListenSocket hListenSock =
	/// SteamNetworkingSockets()->CreateListenSocketIP( localAddress, 1, &opt );
	///
	/// When accepting an incoming connection, there is no atomic way to switch
	/// the callback.  However, if the connection is DOA, AcceptConnection()
	/// will fail, and you can fetch the state of the connection at that time.
	///
	/// If all connections and listen sockets can use the same callback, the
	/// simplest method is to set it globally before you create any listen
	/// sockets or connections.
	k_ESteamNetworkingConfig_Callback_ConnectionStatusChanged = 201,

	/// [global FnSteamNetAuthenticationStatusChanged] Callback that will be
	/// invoked when our auth state changes.  If you use this, install the
	/// callback before creating any connections or listen sockets, and don't
	/// change it. See:
	/// ISteamNetworkingUtils::SetGlobalCallback_SteamNetAuthenticationStatusChanged
	k_ESteamNetworkingConfig_Callback_AuthStatusChanged = 202,

	/// [global FnSteamRelayNetworkStatusChanged] Callback that will be invoked
	/// when our auth state changes.  If you use this, install the callback
	/// before creating any connections or listen sockets, and don't change it.
	/// See:
	/// ISteamNetworkingUtils::SetGlobalCallback_SteamRelayNetworkStatusChanged
	k_ESteamNetworkingConfig_Callback_RelayNetworkStatusChanged = 203,

	/// [global FnSteamNetworkingMessagesSessionRequest] Callback that will be
	/// invoked when a peer wants to initiate a
	/// SteamNetworkingMessagesSessionRequest. See:
	/// ISteamNetworkingUtils::SetGlobalCallback_MessagesSessionRequest
	k_ESteamNetworkingConfig_Callback_MessagesSessionRequest = 204,

	/// [global FnSteamNetworkingMessagesSessionFailed] Callback that will be
	/// invoked when a session you have initiated, or accepted either fails to
	/// connect, or loses connection in some unexpected way. See:
	/// ISteamNetworkingUtils::SetGlobalCallback_MessagesSessionFailed
	k_ESteamNetworkingConfig_Callback_MessagesSessionFailed = 205,

	/// [global FnSteamNetworkingSocketsCreateConnectionSignaling] Callback that
	/// will be invoked when we need to create a signaling object for a
	/// connection initiated locally.  See: ISteamNetworkingSockets::ConnectP2P,
	/// ISteamNetworkingMessages.
	k_ESteamNetworkingConfig_Callback_CreateConnectionSignaling = 206,

	/// [global FnSteamNetworkingFakeIPResult] Callback that's invoked when
	/// a FakeIP allocation finishes.  See:
	...
	//	/// [listen socket int32] When you create a P2P listen socket, we will
	// automatically
	//	/// open up a UDP port to listen for LAN connections.  LAN connections
	// can be made
	//	/// without any signaling: both sides can be disconnected from the
	// Internet.
	//	///
	//	/// This value can be set to zero to disable the feature.
	//	k_ESteamNetworkingConfig_P2P_Discovery_Server_LocalPort = 101,
	//
	//	/// [connection int32] P2P connections can perform broadcasts looking
	// for the peer
	//	/// on the LAN.
	//	k_ESteamNetworkingConfig_P2P_Discovery_Client_RemotePort = 102,

	/// [connection string] Comma-separated list of STUN servers that can be
	/// used for NAT piercing.  If you set this to an empty string, NAT piercing
	/// will not be attempted.  Also if "public" candidates are not allowed for
	...
	/// [int32 global] If the first N pings to a port all fail, mark that port
	/// as unavailable for a while, and try a different one.  Some ISPs and
	/// routers may drop the first packet, so setting this to 1 may greatly
	/// disrupt communications.
	k_ESteamNetworkingConfig_SDRClient_ConsecutitivePingTimeoutsFailInitial
	    = 19,

	/// [int32 global] If N consecutive pings to a port fail, after having
	/// received successful communication, mark that port as unavailable for a
	/// while, and try a different one.
	k_ESteamNetworkingConfig_SDRClient_ConsecutitivePingTimeoutsFail = 20,

	/// [int32 global] Minimum number of lifetime pings we need to send, before
	/// we think our estimate is solid.  The first ping to each cluster is very
	...
	/// local port. By default, we open up a new UDP socket (on a different
	/// local port) for each relay.  This is slightly less optimal, but it works
	/// around some routers that don't implement NAT properly.  If you have
	/// intermittent problems talking to relays that might be NAT related, try
	/// toggling this flag
	k_ESteamNetworkingConfig_SDRClient_SingleSocket = 22,

	/// [global string] Code of relay cluster to force use.  If not empty, we
	/// will only use relays in that cluster.  E.g. 'iad'
	k_ESteamNetworkingConfig_SDRClient_ForceRelayCluster = 29,

	/// [connection string] For debugging, generate our own (unsigned) ticket,
	/// using the specified  gameserver address.  Router must be configured to
	/// accept unsigned tickets.
	k_ESteamNetworkingConfig_SDRClient_DebugTicketAddress = 30,

	/// [global string] For debugging.  Override list of relays from the config
	/// with this set (maybe just one).  Comma-separated list.
	k_ESteamNetworkingConfig_SDRClient_ForceProxyAddr = 31,

	/// [global string] For debugging.  Force ping times to clusters to be the
	/// specified values.  A comma separated list of <cluster>=<ms> values. E.g.
	/// "sto=32,iad=100"
	///
	/// This is a dev configuration value, you probably should not let users
	/// modify it in production.
...
const int k_nSteamNetworkingConfig_P2P_Transport_ICE_Enable_Default
    = -1;   // Special value - use user defaults
...
const int k_nSteamNetworkingConfig_P2P_Transport_ICE_Enable_Relay
    = 1;   // Relayed connection via TURN server.
...
const int k_nSteamNetworkingConfig_P2P_Transport_ICE_Enable_Public
    = 4;   // STUN reflexive addresses, or host address that isn't a "private"
           // address
...
/// connection really starts doing anything. Creating the object and then
/// setting the options "immediately" after creation doesn't work completely,
/// because network packets could be received between the time the object is
/// created and when the options are applied.  To set options at creation time
/// in a reliable way, they must be passed to the creation function.  This
/// structure is used to pass those options.
///
...
/// options and call ISteamNetworkingUtils::SetConfigValueStruct, where the
/// scope arguments are supplied by the object being created.
struct SteamNetworkingConfigValue_t {
	...
	inline void
	SetString(ESteamNetworkingConfigValue eVal,
	          const char *data)   // WARNING - Just saves your pointer.  Does
...
enum ESteamNetworkingGetConfigValueResult {
	k_ESteamNetworkingGetConfigValue_BadValue
	    = -1,   // No such configuration value
	k_ESteamNetworkingGetConfigValue_BadScopeObj
	    = -2,   // Bad connection handle, etc
	k_ESteamNetworkingGetConfigValue_BufferTooSmall
	    = -3,   // Couldn't fit the result in your buffer
	k_ESteamNetworkingGetConfigValue_OK = 1,
	k_ESteamNetworkingGetConfigValue_OKInherited
	    = 2,   // A value was not set at this level, but the effective
	           // (inherited) value was returned.
...
enum ESteamNetworkingSocketsDebugOutputType {
	...
	k_ESteamNetworkingSocketsDebugOutputType_Bug
	    = 1,   // You used the API incorrectly, or an internal error happened
	k_ESteamNetworkingSocketsDebugOutputType_Error
	    = 2,   // Run-time error condition that isn't the result of a bug. (E.g.
	           // we are offline, cannot bind a port, etc)
...
inline SteamNetworkingPOPID
CalculateSteamNetworkingPOPIDFromString(const char *pszCode) {
	// OK we made a bad decision when we decided how to pack 3-character codes
	// into a uint32.  We'd like to support 4-character codes, but we don't want
	// to break compatibility.  The migration path has some subtleties that make
	// this nontrivial, and there are already some IDs stored in SQL.  Ug, so
	// the 4 character code "abcd" will be encoded with the digits like
	// "0xddaabbcc".
	//
	// Also: we don't currently use 1- or 2-character codes, but if ever do in
	// the future, let's make sure don't read past the end of the string and
	// access uninitialized memory.  (And if the string is empty, we always want
	// to return 0 and not read bytes past the '\0'.)
	//
	// There is also extra paranoia to make sure the bytes are not treated as
	// signed.
	SteamNetworkingPOPID result = (uint32)(uint8)pszCode[0] << 16U;
	if (pszCode[1]) {
		result |= ((uint32)(uint8)pszCode[1] << 8U);
		if (pszCode[2]) {
			result |= (uint32)(uint8)pszCode[2]
			          | ((uint32)(uint8)pszCode[3] << 24U);
		}
	}
	return result;
}
...
template<int N>
inline void GetSteamNetworkingLocationPOPStringFromID(SteamNetworkingPOPID id,
                                                      char (&szCode)[N]) {
	static_assert(N >= 5,
	              "Fixed-size buffer not big enough to hold SDR POP ID");
	szCode[0] = char(id >> 16U);
	szCode[1] = char(id >> 8U);
	szCode[2] = char(id);
	szCode[3]
	    = char(id >> 24U);   // See comment above about deep regret and sadness
	szCode[4] = 0;
}

/// The POPID "dev" is used in non-production environments for testing.
const SteamNetworkingPOPID k_SteamDatagramPOPID_dev
...
inline bool SteamNetworkingIPAddr::IsIPv4() const {
	return m_ipv4.m_8zeros == 0 && m_ipv4.m_0000 == 0
	       && m_ipv4.m_ffff == 0xffff;
}
inline uint32 SteamNetworkingIPAddr::GetIPv4() const {
	return IsIPv4() ? ((uint32(m_ipv4.m_ip[0]) << 24)
	                   | (uint32(m_ipv4.m_ip[1]) << 16)
...
inline bool SteamNetworkingIPAddr::IsLocalHost() const {
	return (m_ipv4.m_8zeros == 0 && m_ipv4.m_0000 == 0 && m_ipv4.m_ffff == 0
	        && m_ipv6[12] == 0 && m_ipv6[13] == 0 && m_ipv6[14] == 0
	        && m_ipv6[15] == 1)
	       || (GetIPv4() == 0x7f000001);
}
inline bool
SteamNetworkingIPAddr::operator==(const SteamNetworkingIPAddr &x) const {
...
inline void
SteamNetworkingIdentity::SetIPAddr(const SteamNetworkingIPAddr &addr) {
...
inline bool SteamNetworkingIdentity::IsLocalHost() const {
	return m_eType == k_ESteamNetworkingIdentityType_IPAddress
	       && m_ip.IsLocalHost();
}
...
inline const char *SteamNetworkingIdentity::GetGenericString() const {
	return m_eType == k_ESteamNetworkingIdentityType_GenericString
	           ? m_szGenericString
	           : NULL;
}
inline bool SteamNetworkingIdentity::SetGenericBytes(const void *data,
                                                     size_t cbLen) {
...
inline bool
SteamNetworkingIdentity::operator==(const SteamNetworkingIdentity &x) const {
